...
IF %1.==--help. (
    ...
    ECHO    --without-nmea_count_aismsgtypes  do not build nmea_count_aismsgtypes.exe
    ECHO    --without-aisnmea_selftest  do not build aisnmea_selftest.exe
    GOTO :eof
)
ECHO Configuring aisnmea...
...
ECHO. >> platform.h
ECHO #define AISNMEA_HAVE_WINDOWS 1>> platform.h
...
IF EXIST "..\..\..\libzmq" (
    ...
    ECHO aisnmea cannot build without libzmq
...
IF EXIST "..\..\..\czmq" (
    ...
    ECHO aisnmea cannot build without czmq
...
:with_draft
    ...
    ECHO #define AISNMEA_BUILD_DRAFT_API 1>>platform.h
    GOTO end_draft
:no_draft
    ECHO Building without draft API (stable + legacy API)
    ECHO #undef AISNMEA_BUILD_DRAFT_API 1>>platform.h
:end_draft
ECHO. >> platform.h
